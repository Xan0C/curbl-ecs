...
export class EntityComponentManager {
    ...
    addEntity<T extends object>(entity: T, components?: { [x: string]: Component }, silent = false, silentWorker = false): T & Entity {
        const injectedEntity = injectEntity(entity);
        this.entities[injectedEntity.id] = injectedEntity;
        ...
        this.updateComponentBitmask(injectedEntity);
        if (!silent) {
            this.events.emit(ECM_EVENTS.ENTITY_ADDED, injectedEntity);
        }
        if (!silentWorker) {
            this.events.emit(ECM_WORKER_EVENTS.ENTITY_ADDED, injectedEntity);
        }
    ...
    updateEntities(entities: EntityMap, silent?: boolean, silentWorker?: boolean): void {
        ...
        if (!silent) {
            this.events.emit(ECM_EVENTS.ENTITIES_UPDATED, entities);
        }
        if (!silentWorker) {
            this.events.emit(ECM_WORKER_EVENTS.ENTITIES_UPDATED, entities);
        }
    }
    ...
    removeEntity<T extends EntityProp>(entity: T, silent = false, silentWorker?: boolean): T {
        if (this.hasEntity(entity)) {
            if (!silent) {
                this.events.emit(ECM_EVENTS.ENTITY_REMOVED, entity);
            }
            if (!silentWorker) {
                this.events.emit(ECM_WORKER_EVENTS.ENTITY_REMOVED, entity);
            }
    ...
    removeAllEntities(): Entity[] {
        const entities = [];
        const keys: string[] = Object.keys(this.entities);
        for (let i = 0, entity: Entity; (entity = this.entities[keys[i]]); i++) {
    ...
    addComponent<T extends object>(entity: EntityProp, component: T, silent = false, silentWorker?: boolean): void {
        ...
        if (!silent && this.hasEntity(entity)) {
            this.events.emit(ECM_EVENTS.COMPONENT_ADDED, entity, injectedComponent);
        }
        if (!silentWorker && this.hasEntity(entity)) {
            this.events.emit(ECM_WORKER_EVENTS.COMPONENT_ADDED, entity, injectedComponent);
        }
    }
    ...
    removeComponent<T extends Component>(
        entity: EntityProp,
        component: { new (...args): T } | string,
        silent = false,
        silentWorker?: boolean
    ): boolean {
        let comp: Component;
        ...
        if (comp) {
            ...
            if (!silent && this.hasEntity(entity)) {
                this.events.emit(ECM_EVENTS.COMPONENT_REMOVED, entity, comp);
            }
            if (!silentWorker && this.hasEntity(entity)) {
                this.events.emit(ECM_WORKER_EVENTS.COMPONENT_REMOVED, entity, comp);
            }
            comp.remove();
